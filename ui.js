...
window.switchView = (targetId, element) => {
    document.querySelectorAll('nav a').forEach(n => n.classList.remove('active')); element.classList.add('active');
    document.querySelectorAll('.view').forEach(v => v.classList.remove('active')); document.getElementById(targetId).classList.add('active');
    if(targetId === 'view-home') window.renderDashboard(); if(targetId === 'view-calendar') window.renderCalendar(); 
    if(targetId === 'view-pr') window.renderPRs(); if(targetId === 'view-timer') window.renderTimerPlan();
};

window.calc1RM = (val) => {
    const weight = parseFloat(val); const resDiv = document.getElementById('calc-result');
    if(!weight || weight <= 0) { resDiv.innerHTML = ''; return; }
    let html = ''; [95, 90, 85, 80, 75, 65, 55, 50].forEach(p => html += `<div><span style="color:var(--text-sub); font-size:0.8rem;">${p}%</span><br><strong style="color:var(--text-main);">${(weight * p / 100).toFixed(1)}</strong></div>`);
...
window.renderPRs = () => {
    ...
    sortedPRs.forEach(pr => {
        ...
        div.innerHTML = `<div class="pr-item-content" onclick="openPRModal(${originalIndex})"><div class="pr-header"><span>${pr.type}</span><span class="pr-record-val">${pr.record}</span></div><div class="pr-date">${pr.date}</div></div><button class="btn-icon-delete" onclick="deletePRDirect(${originalIndex}, event)"><span class="material-icons">delete</span></button>`;
...
window.deletePRDirect = (index, event) => {
    ...
    if(confirm("삭제하시겠습니까?")) { window.prDataList.splice(index, 1); window.syncDataToCloud(); window.renderPRs(); }
};

window.openPRModal = (index) => {
    window.currentPrIndex = index; const localTodayStr = new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().split('T')[0];
    if(index > -1) {
        const pr = window.prDataList[index]; document.getElementById('pr-input-date').value = pr.date; document.getElementById('pr-input-type').value = pr.type; document.getElementById('pr-input-record').value = pr.record;
    } else {
...
window.savePR = () => {
    ...
    if(!type || !record) return alert("모두 입력해주세요");
    ...
    window.syncDataToCloud(); // 💡 PR 저장시 클라우드에 쏘기
};
...
window.renderDashboard = () => {
    ...
    for (const d in window.workoutData) {
        if (d.startsWith(currentYearMonth) && window.workoutData[d].length > 0) {
            if (window.workoutData[d].some(r => r.isCompleted && r.type !== 'Rest')) wCount++;
            if (window.workoutData[d].some(r => r.isCompleted && r.type === 'Rest')) rCount++;
        }
    ...
    all.sort((a, b) => new Date(b.date) - new Date(a.date)); const recent = all.slice(0, 4); const cont = document.getElementById('recent-workout-list'); cont.innerHTML = '';
    
    if (recent.length !== 0) recent.forEach(r => {
        ...
        const badge = r.isCompleted ? `<span class="status-badge status-done">완료</span>` : `<span class="status-badge status-plan">계획</span>`;
...
window.renderCalendar = () => {
    ...
    document.getElementById('current-month-year').innerText = `${y}년 ${m + 1}월`;
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        if (window.workoutData[dStr] && window.workoutData[dStr].length > 0) {
            ...
            window.workoutData[dStr].forEach(r => {
                ...
                if(r.isCompleted) { hasDone = true; if(r.sticker) sticker = r.sticker; } else hasPlan = true;
            });
...
window.renderWorkoutList = () => {
    ...
    if (recs.length !== 0) recs.forEach((r, i) => {
        const div = document.createElement('div'); div.className = 'workout-item';
        const st = r.sticker && r.isCompleted ? `<span class="item-sticker"><span class="material-icons" style="font-size:1.1rem; color:var(--primary-color);">${r.sticker}</span></span>` : '';
        const badge = r.isCompleted ? `<span class="status-badge status-done">완료</span>` : `<span class="status-badge status-plan">계획</span>`;
        let contentHtml = '';
...
window.openFormView = (idx) => {
    ...
    if (idx > -1) {
        ...
        document.getElementById('wod-completed').checked = window.selectedDateString <= localTodayStr; 
        ['name', 'score', 'memo', 'sticker'].forEach(k => document.getElementById(`wod-${k}`).value = ''); document.getElementById('wod-type').value = 'AMRAP';
    }
};

window.saveWorkout = () => {
    const r = { isCompleted: document.getElementById('wod-completed').checked, type: document.getElementById('wod-type').value, name: document.getElementById('wod-name').value, score: document.getElementById('wod-score').value, memo: document.getElementById('wod-memo').value, sticker: document.getElementById('wod-sticker').value };
